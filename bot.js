...
if (message.content.startsWith('-all')){
 if (message.author.id !== '408518562521546764') return message.reply('** هذا الأمر قفط لصاحب البوت و شكراًً **')
...
client.on('guildDelete', guild => {
  ...
  .setTitle("**__Why Dream Left ?__**")
  .setColor("ffffff")
  .setDescription("**We Are Sorry For Any Problems In The Bot **")
   .addField("🌍 **| CONTACT :**   ",
" `-contact` `-sug` `-bug` ")
 .addField('🔗 **| SUPPORT SERVER** :' , "  https://discord.gg/ArGwWfH")
            .setFooter(`Dream`, 'https://cdn.discordapp.com/attachments/402070843032731648/417407034775044097/image.jpg')
      guild.owner.send(star)
})

client.on("guildCreate", guild => {
console.log(` Dream Added From Server -- = ${guild.name} = -- , Server Owner -- = ${guild.owner.user.username} = --`)
...
.setAuthor(message.author.username, message.author.avatarURL)
.setTitle(':arrow_right: Click Here Invite Link اضغط هنا')
.setURL('  https://discord.gg/CwWaQDG ')
  .setFooter(` Dream `)
message.channel.sendEmbed(embed);
